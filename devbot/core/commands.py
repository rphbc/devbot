...
class BaseCommand:
    ...
    def _create_arguments_regex(self, arguments):
        regex = r''
    ...
    def _search_actions(self, content):
    ...
    def _get_action_arguments(self, action, content):
        action_data = self.actions[action]
        args_regex = self._create_arguments_regex(arguments=action_data[
            'arguments'])
        if not args_regex:
            return {}
    ...
    def run_command(self, command_text):
        action, action_content  = self._search_actions(command_text)
        if not action:
            return False
        action_args = self._get_action_arguments(action, action_content)
        return getattr(self, action)(**action_args)
